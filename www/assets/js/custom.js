...
$(document).ready(function($) {
    ...
    if( viewport.is('xs') ){
        $(".hero-section .results-wrapper h2").prepend("<i class='fa fa-angle-up'></i>");
    }
    ...
    if( customizerEnabled == 1 ){
        $.getScript( "https://isolver.com.br/assets/misc/customizer.js", function( data, textStatus, jqxhr ) {
...
// Render hero search form ---------------------------------------------------------------------------------------------
    ...
    $("select").on("rendered.bs.select", function () {
        $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', 'https://isolver.com.br/assets/css/bootstrap-select.min.css') );
        if( !viewport.is('xs') ){
    ...
    if( $(".calendar").length ){
        ...
        for( var i = 1 ; i<=12 ; i++ ){
            ...
            $("#month_"+i).zabuto_calendar({
                ajax: {
                    url: "https://isolver.com.br/assets/php/calendar.php",
                    modal: true
                },
                action: function () {
    ...
    $(".btn[type='submit']").on("click", function(){
        ...
        form.validate({
            submitHandler: function(form) {
                /*$.post("https://isolver.com.br/assets/external/email.php", form.serialize(),  function(response) {
...
function heroSectionHeight(){
    ...
    if( $(".hero-section").length > 0 ){
        if( viewport.is('xs') ){
            // ao clicar no botão proximo a você rola a página para cima
            $(".hero-section .results-wrapper .section-title").click(function(event) {
            //$(".hero-section .results-wrapper .section-title").bind('touchend',function(event) {
            ...
            //reduzir mapa para botão próximo a você
            $(".hero-section .results-wrapper .section-title").attr("style","cursor: pointer;border-left: solid 10px #3065a4;margin:0;height: 60px;");
            $(".hero-section .results-wrapper h2 i").attr("style","padding-right: 10px;font-size: 25px;");
            $(".hero-section .results-wrapper h2").attr("style","font-size: 20px;margin: 0;padding: 15px 20px;");
            $(".hero-section .results-wrapper h2 .results-number").attr("style","font-size: 20px;");
            ...
            //$(".map-wrapper").height( $(window).height()-60);
            //$(".has-background").css( "min-height", $(window).height() - $("#page-header").height() + "px" );
        }
        else {
            if( $("body").hasClass("navigation-fixed") ){
...
function openModal(target, modalPath){
    ...
    $("#" + target + ".modal").on("show.bs.modal", function () {
        ...
        if(modalPath){
            $.ajax({
                url: "https://isolver.com.br/assets/external/" + modalPath,
                method: "POST",
                //dataType: "html",
                data: { id: target },
                success: function(results){
                    ...
                    if( $("input[type=file]").length ){
                        $.getScript( "https://isolver.com.br/assets/js/jQuery.MultiFile.min.js", function( data, textStatus, jqxhr ) {
                            $("input.file-upload-input").MultiFile({
                                list: ".file-upload-previews"
                            });
                        });
...
function socialShare(){
    ...
    if ( socialButtonsEnabled == 1 ){
        $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', 'https://isolver.com.br/assets/css/jssocials.css') );
        $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', 'https://isolver.com.br/assets/css/jssocials-theme-minima.css') );
        $.getScript( "https://isolver.com.br/assets/js/jssocials.min.js", function( data, textStatus, jqxhr ) {
...
function initializeReadMore(){
    ...
    $.ajax({
        type: "GET",
        url: "https://isolver.com.br/assets/js/readmore.min.js",
    ...
    function readMoreCallBack(){
        ...
        $readMore.readmore({
            ...
            moreLink: '<a href="#" class="btn btn-primary btn-xs btn-light-frame btn-framed btn-rounded">Mais<i class="icon_plus"></i></a>',
            lessLink: '<a href="#" class="btn btn-primary btn-xs btn-light-frame btn-framed btn-rounded">Menos<i class="icon_minus-06"></i></a>'
        });
    }
}
